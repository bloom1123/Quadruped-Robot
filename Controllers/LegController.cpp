...
void LegController<T>::updateData(LegData* legData) {
  for (int leg = 0; leg < 4; leg++) {
    ...
    // J and p 雅可比和足端位置
    // _quadruped.computeLegJacobianAndPosition(datas[leg].q, &(datas[leg].J), &(datas[leg].p), leg);
...
void LegController<T>::updateCommand(LegCommand* legCommand) {
  for (int leg = 0; leg < 4; leg++) {
    ...
    // 记录扭矩
    legCommand->tau_abad_ori[leg] = legTorque(0);
    legCommand->tau_hip_ori[leg] = legTorque(1);
    legCommand->tau_knee_ori[leg] = legTorque(2);
    
    // set command:
    legCommand->tau_abad_ff[leg] =
        0.2f * (commands[leg].qDes(0) - datas[leg].q(0)) +
        0.2f * (commands[leg].qdDes(0) - datas[leg].qd(0)) + legTorque(0);

    legCommand->tau_hip_ff[leg] =
        0.2f * (commands[leg].qDes(1) - datas[leg].q(1)) +
        0.2f * (commands[leg].vDes(1) - datas[leg].qd(1)) + legTorque(1);

    legCommand->tau_knee_ff[leg] =
        0.2f * (commands[leg].qDes(2) - datas[leg].q(2)) +
        0.2f * (commands[leg].qdDes(2) - datas[leg].qd(2)) + legTorque(2);
  }
}
