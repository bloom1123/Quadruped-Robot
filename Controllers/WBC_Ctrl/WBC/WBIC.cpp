#include "WBIC.hpp"
#include "../../../third-party/eigen3/Eigen/LU"
#include "../../../third-party/eigen3/Eigen/SVD"
...
void WBIC<T>::MakeTorque(DVec<T>& cmd, void* extra_input) {
  ...
  DMat<T> JcBar;
  DMat<T> Npre;
  //printf("Dim RF after optmization setting: %ld\n",_dim_rf);
  if (_dim_rf > 0) {
    // Contact Setting
    _ContactBuilding();
  ...
  _SetEqualityConstraint(qddot_pre);//critic
...
void WBIC<T>::_ContactBuilding() {
  ...
  for (size_t i(1); i < (*_contact_list).size(); ++i) {
    ...
    //cout << "\nRot trans in WBIC: " << Rpla.transpose() << endl;
  ...
  //pretty_print(_Uf_ieq_vec, std::cout, "[WBIC] _Uf_ieq_vec");
