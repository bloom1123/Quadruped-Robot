...
void LinearKFPositionVelocityEstimator<T>::run() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    Vec3<T> dp_f = Rbod * (this->_stateEstimatorData.result->omegaBody.cross(p_rel) + dp_rel);  //足端速度在世界坐标系中
    ...
    // T phase = fmin(*this->_stateEstimatorData.contactPhase, T(1)); // after remove contact state estimator
